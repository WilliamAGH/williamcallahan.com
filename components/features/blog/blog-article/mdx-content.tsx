 ...
 * It extends standard `img` component props but omits some that are handled internally
 * for optimization and styling (height, width, loading, style).
 */
interface ArticleImageProps
  extends Omit<ComponentProps<"img">, "height" | "width" | "loading" | "style"> {
  /** Optional caption to display below the image. */
  caption?: string;
  /**
   * Specifies the display size of the image, affecting its max-width within the layout.
   * Defaults to 'full'.
   * 'full': Typically constrained by the main content column width.
   * 'medium': A moderately sized image, smaller than the full content width.
   * 'small': A smaller image, suitable for inline or tighter layouts.
   */
  size?: "full" | "medium" | "small";
  /**
   * If true, the image will be marked as high priority for loading (e.g., for Largest Contentful Paint images).
   * Defaults to false.
   */
  priority?: boolean;
}
...
/**
 * Props for the {@link ArticleGallery} component.
 */
interface ArticleGalleryProps {
  /** The content of the gallery, typically a series of images or other media elements. */
  children: ReactNode;
  /** Optional CSS class names to apply to the gallery container for additional styling. */
  className?: string;
}
...
/**
 * Props for the main {@link MDXContent} component.
 */
interface MDXContentProps {
  /**
   * The serialized MDX content object.
   * This is typically generated by `serialize` from `next-mdx-remote/serialize`.
   */
  ...
  const components: MDXRemoteProps["components"] = useMemo(
    () => ({
      ...
      p: (props: ComponentProps<"p">) => (
        <p className="text-gray-700 dark:text-gray-300 text-base" {...props} />
      ),
