...
export function SocialCardClient({ social }: SocialCardProps): JSX.Element {
  ...
  const getProfileFallbackImage = useCallback((networkLabel: string): string => {
    try {
      ...
      if (networkLabel.includes('Bluesky')) return 'https://cdn.bsky.app/img/avatar/plain/did:plc:6y3lzhinepgneechfrv3w55d/bafkreicuryva5uglksh2tqrc5tu66kwvnjwnpd2fdb6epsa6fjhhdehhyy@jpeg';
  ...
  const fetchSocialImages = useCallback(async (url: string) => {
    ...
    try {
      ...
      interface OgImageApiResponse { profileImageUrl?: string; domainImageUrl?: string; }
  ...
  if (!mounted) {
    return <div className="relative flex flex-col bg-white/50 dark:bg-gray-800/50 backdrop-blur-lg ring-0 rounded-3xl overflow-hidden shadow-xl h-full" />;
  }
  ...
  return (
    <div className={`relative flex flex-col bg-white/50 dark:bg-gray-800/50 backdrop-blur-lg ring-0 rounded-3xl overflow-hidden shadow-xl border border-transparent group ${cardBrandClass} h-full`}>
      <div className="relative w-full aspect-video overflow-hidden rounded-t-3xl bg-gray-100 dark:bg-gray-800">
        ...
        <div className="absolute inset-0 w-full h-full overflow-hidden rounded-t-3xl">
          {domainImageUrl && (
            <div className="absolute inset-0 w-full h-full overflow-hidden">
              <a href={href} target="_blank" rel="noopener noreferrer" className="absolute inset-0 z-20 w-full h-full cursor-pointer block" title={`Visit ${profileName} profile page`}>
                <span className="sr-only">Visit {serviceName || 'social'} profile</span>
              </a>
              <div className="absolute inset-0 w-full h-full">
                {domain.includes('linkedin') ? (
                  <Image src={domainImageUrl} alt={serviceName ? `${serviceName} branding` : "Social media branding"} className="w-full h-full object-cover linkedin-banner" fill unoptimized={true} onError={() => setImageError(true)} />
                ) : (
                  <Image src={domainImageUrl} alt={serviceName ? `${serviceName} branding` : "Social media branding"} className="w-full h-full object-cover social-banner" fill unoptimized={true} onError={() => setImageError(true)} />
                )}
              </div>
            </div>
          )}
          ...
          {profileImageUrl && (
            <div className="absolute bottom-4 right-4 z-10">
              <a href={href} target="_blank" rel="noopener noreferrer" className="block relative w-12 h-12 md:w-16 md:h-16 cursor-pointer rounded-full" title={`Visit ${profileName} profile page`}>
                <div className="absolute inset-0 rounded-full overflow-hidden shadow-lg border-2 border-white/70 dark:border-gray-700/70">
                  <Image src={profileImageUrl} alt={serviceName ? `${serviceName} profile` : "Social media profile"} fill unoptimized={true} priority={true} sizes="(max-width: 767px) 48px, 64px" className="object-cover" onError={() => setProfileImageUrl(getProfileFallbackImage(label))} />
                </div>
        ...
        <ExternalLink href={href} showIcon={false} className="absolute bottom-3 left-3 bg-white/80 dark:bg-gray-800/80 px-3 py-1 flex items-center space-x-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
      ...
      <div className="flex-1 p-6 flex flex-col gap-3.5">
        <a href={href} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2.5 group/title" title={`Visit ${profileName} profile page`}>
          <Icon className="w-6 h-6 text-gray-700 dark:text-gray-300 group-hover/title:text-blue-600 transition-colors" />
          <span className="text-2xl font-semibold text-gray-900 dark:text-white group-hover/title:text-blue-600 transition-colors">{serviceName}</span>
        </a>
        <p className="flex-1 text-gray-700 dark:text-gray-300 text-base leading-6 overflow-hidden">{getNetworkDescription(label)}</p>
        <div className="mt-auto space-y-2 text-sm text-gray-500 dark:text-gray-400">
          <div className="flex items-center gap-1">
            <a href={href} target="_blank" rel="noopener noreferrer" className="font-mono hover:text-blue-600 transition-colors" title={`Visit ${profileName} profile page`}>{getUserHandle(href)}</a>
          </div>
        </div>
        <div className="flex flex-wrap gap-2.5 mt-3 pt-4 pb-2 border-t border-gray-200 dark:border-gray-700">
          <span className="inline-block px-3 py-1 rounded-full bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 text-xs font-medium transition-colors hover:bg-indigo-200 dark:hover:bg-indigo-800/60 transform hover:scale-102">{getNetworkCategory(label)}</span>
          {emphasized && (<span className="inline-block px-3 py-1 rounded-full bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 text-xs font-medium">Primary</span>)}
        </div>
      </div>
    </div>
  );
