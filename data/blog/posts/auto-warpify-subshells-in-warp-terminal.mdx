...
<BackgroundInfo title="Important: Configure this on the host you're connecting to">
  <div>
    ...
    you're connecting to (for example: a Docker container, a remote SSH server, or a VM). Place it in that user's shell
    startup file (<code>{"~/.bashrc"}</code> or <code>{"~/.bash_profile"}</code> for bash, <code>{"~/.zshrc"}</code>{" "}
    for zsh).
  </div>
  <ul>
    <li>
      <strong>SSH:</strong> edit the remote user's RC file on the server you SSH into.
    </li>
    <li>
      <strong>Docker:</strong> edit the container image or the running container's RC file.
    </li>
    <li>
      <strong>Local nested shells:</strong> add it to your local RC if you want local subshells warpified.
    </li>
  </ul>
</BackgroundInfo>
