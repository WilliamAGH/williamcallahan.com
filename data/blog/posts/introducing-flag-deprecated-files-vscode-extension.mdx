...
Working in large repositories, I often flag files as deprecated with JSDoc/JavaDoc/Docstring tags when we're planning to phase them out. The problem? There was no easy way to _see_ which files were deprecated without opening each one. In my case, these are repo's with a thousand + files, so it was a real pain.
