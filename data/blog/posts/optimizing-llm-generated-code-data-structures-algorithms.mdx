...
> **Context (2025 Q4):** These SQL examples assume PostgreSQL 15 running behind Node.js 22 with `pg@8.12`, using `pool` as an instance of `pg.Pool`. Adjust parameter binding syntax if you're using a different driver.

### Common LLM Code (N+1 Queries)
```typescript
// LLM often generates this disaster
async function getUsersWithPosts(userIds: number[]) {
  const results: Array<{ user: User; posts: Post[] }> = [];
...
```typescript
// Better approach with a single round-trip
async function getUsersWithPosts(userIds: number[]) {
  const query = `
    SELECT
      u.id,
  ...
  const userMap = new Map<number, { id: number; name: string; email: string; posts: Post[] }>();
...
const GET_USERS_WITH_POSTS = `
  ...
  SELECT
    ...
    u.name,
    u.email,
    COALESCE(posts.posts, '[]'::jsonb) AS posts,
    COALESCE(followers.followers, '[]'::jsonb) AS followers
  FROM users u
  JOIN input_ids ON input_ids.id = u.id
  LEFT JOIN LATERAL (
    SELECT jsonb_agg(row_to_json(p) ORDER BY p.created_at DESC) AS posts
    FROM posts p
    WHERE p.user_id = u.id
  ) posts ON TRUE
  LEFT JOIN LATERAL (
    SELECT jsonb_agg(row_to_json(f)) AS followers
    FROM followers f
...
async function getUsersWithPosts(userIds: number[]) {
  ...
  try {
    const result = await client.query<{ id: number; name: string; email: string; posts: unknown; followers: unknown }>(
      {
        name: 'get-users-with-posts',
        text: GET_USERS_WITH_POSTS,
        values: [userIds],
      },
    );
    ...
    return result.rows.map(row => ({
      ...
      name: row.name,
      email: row.email,
      posts: (row.posts as unknown[] | null) ?? [],
      followers: (row.followers as unknown[] | null) ?? [],
    }));
  } finally {
    client.release();
  }
...
app.get('/api/user/:id', async (req, res) => {
  ...
  const requested = new Set((fieldsParam ?? 'basic').split(',').map(field => field.trim()));
...
const userLoader = new DataLoader(async (userIds: readonly number[]) => {
  const { rows } = await pool.query(GET_USERS_WITH_POSTS, [Array.from(userIds)]);
...
app.get('/api/user/:id', async (req, res) => {
  ...
  // Cache for 5 minutes
  await redis.setEx(`user:${userId}`, 300, JSON.stringify(userData)); // redis@4.6 camelCase API
...
| LRU Cache | Array | `lru-cache@10` (doubly linked list + Map) | O(n) → O(1) |
...
– Did I miss any critical data-structure pitfalls?
– Are there scenarios where Sets/Maps are *not* the right fix?
