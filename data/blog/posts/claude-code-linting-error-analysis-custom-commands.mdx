...
<BackgroundInfo title="Intro to Claude Code in Your CLI">
  Stop context-switching between your editor and terminal. Claude Code's built-in **/analyze** plus custom slash
  commands like **/project:lint** bring static analysis, error tracing, and project-specific helpers directly into your
  CLI workflow.
</BackgroundInfo>
...
<CollapseDropdown summary="0.1 Install & launch" id="install-launch">
  - **Install globally**: ```bash npm install -g @anthropic-ai/claude-code ``` - **Launch in any repository**: ```bash
  claude ``` - **Docs & download**: [Claude Code CLI](https://docs.anthropic.com/en/docs/claude-code/quickstart)
</CollapseDropdown>
...
<CollapseDropdown summary="1.1 /lint at a glance" id="lint-at-a-glance">
  ...
  **Syntax**: `/lint [path] [--fix]` - **Note**: Claude displays the scope as a suffix, e.g., `/lint (project)` or
  `/lint (user)` based on where you saved the command - **Docs**: [Slash Commands – Custom
...
<CollapseDropdown summary="2.1 /analyze" id="analyze-command">
  - **Purpose**: Given a stack trace or failing test output, Claude pinpoints the source file, probable cause, and
  suggests a minimal fix. - **Usage**: Pipe any stderr/stdout into the command. - **Docs**: [Slash Commands –
  /analyze](https://docs.anthropic.com/en/docs/claude-code/slash-commands)
</CollapseDropdown>
...
STEP 2: Analyze the build output using @mcp**zen**thinkdeep with model="pro".
...
STEP 2: Perform deep analysis using @mcp**zen**thinkdeep with model="pro" and thinking_mode="high".

STEP 3: For test failures:
a) Use @mcp**zen**debug to identify root causes
...
@mcp**zen**thinkdeep model="pro" thinking_mode="high" prompt="
Comment: [paste]
...
if [ "$TOTAL_THREADS" -le 100 ]; then # Simple case
...
else # Complex case: paginate through all unresolved threads
...
THREAD*ID="PRRT*..." # From "Thread ID:" line
...
# The thread ID starts with "PRRT\_" (e.g., "PRRT_kwDONglMk85SSn9V")
...
STEP 1: Use @mcp**github**get_issue to get and output full issue details.

STEP 2: Use @mcp**zen**thinkdeep with model='pro' to deeply analyze the issue.

STEP 3: Search for relevant code using @mcp**github**search_code and Read tools.
...
STEP 3: Create a well-structured GitHub issue using @mcp**github**create_issue with owner="WilliamAGH" repo="williamcallahan.com". Include:
...
- Use @mcp**zen**thinkdeep with model="pro" and thinking_mode="max"
