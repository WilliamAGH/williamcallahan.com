...
<BackgroundInfo title="Critical Security Tips">
  1. **Never commit tokens**: Add `*.env`, `*config.json` with secrets to `.gitignore` 2. **Use
  read-only tokens** where possible for MCP servers 3. **Rotate secrets regularly**: Both tools
  support secret rotation 4. **Enable 2FA** on Doppler and 1Password accounts 5. **Scope permissions
  tightly**: Give MCP servers minimum required access
</BackgroundInfo>
