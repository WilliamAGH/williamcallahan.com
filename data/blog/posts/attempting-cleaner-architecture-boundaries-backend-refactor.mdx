...
The issue truly was remembering the strict boundaries for each layer (knowing exactly where one ends and one should begin). Trul grokking where the boundaries are and sticking to them strictly. You can't see it from this blog post, but there *were/are* a lot of clean boundaries in the codebase already, but the examples given were some of the worst 'all-in-one' files that I should have put strict boundaries on right out of the gate to prevent them from this level of scope creep.
...
### Step 1: Adding architectural guardrails (pragmatically)
...
### Step 2: Introducing a use-case layer
...
Looking back at the `PersonService` that extends `BaseDomainService` (shown earlier), the create method demonstrates all the problems in one place:
