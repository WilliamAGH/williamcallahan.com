...
- **JDK & Maven Setup**: Assumed to be done. For initial JDK/Maven installation and `JAVA_HOME`/`M2_HOME` setup, refer to the <ExternalLink href="/blog/project-level-java-maven-gradle-versioning#part-1-os-level-tool-installation-the-foundation">Core Java & Build Tools Setup Guide</ExternalLink>. This article focuses on Spring Boot project creation and specific Maven usage (like the Spring Boot Maven Plugin).
