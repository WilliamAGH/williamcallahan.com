...
- No global default key (as of 2025-08-24): There's no documented settings key to set a global default output style in `settings.json`; pick per repo.
...
- Note (as of 2025-08-24): Claude Code suggested `/output-style:set`, but the working command is
  `/output-style <styleName>` or use the `/output-style` menu.
  </CollapseDropdown>
...
- **Custom Slash Commands**
  - "Stored prompts," not system prompts. Good for repeatable actions/macros.
    </CollapseDropdown>
