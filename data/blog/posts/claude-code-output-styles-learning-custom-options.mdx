...
<BackgroundInfo title="Overview">
  - What Output Styles are and how they work in Claude Code - The two built‑in styles: Explanatory
  and Learning (with to-do prompts) - Exact commands for switching styles and creating custom ones -
  Where styles are stored and how they persist per project - How Output Styles compare to CLAUDE.md,
  `--append-system-prompt`, Agents, and Slash Commands
</BackgroundInfo>
...
<ul>
  <li>
    Per-project default: Your selection persists for this repo in <code>.claude/settings.local.json</code> and is used
    next time you open it.
  </li>
  <li>
    No global default key (as of 2025-08-24): There's no documented settings key to set a global default output style
    in <code>settings.json</code>; pick per repo.
  </li>
  <li>
    Reuse across repos: Store custom styles in <code>~/.claude/output-styles</code>, then select them once per repo.
  </li>
</ul>
...
<ul>
  <li>
    This selection will persist for the current project via <code>.claude/settings.local.json</code>.
  </li>
  <li>
    Note (as of 2025-08-24): Claude Code suggested <code>/output-style:set</code>, but the working command is
    <code>/output-style &lt;styleName&gt;</code> or use the <code>/output-style</code> menu.
  </li>
</ul>
</CollapseDropdown>
...
<CollapseDropdown summary="Compare: Output Styles vs CLAUDE.md vs --append-system-prompt vs Agents vs Slash Commands">
  <ul>
    <li>
      <strong>Output Styles</strong>
      <ul>
        <li>Replace the main agent’s system prompt (“stored system prompts”).</li>
        <li>Affect the main loop and keep all tools available.</li>
      </ul>
    </li>
    <li>
      <strong>CLAUDE.md</strong>
      <ul>
        <li>Adds content as a user message after the default system prompt.</li>
        <li>Does not change the default system prompt itself.</li>
      </ul>
    </li>
    <li>
      <strong><code>--append-system-prompt</code></strong>
      <ul>
        <li>Appends to the system prompt instead of replacing it.</li>
      </ul>
    </li>
    <li>
      <strong>Agents / Sub-agents</strong>
      <ul>
        <li>Invoked for specific tasks; can pick models/tools and include their own context.</li>
        <li>Output Styles remain the “primary voice”; agents are specialists you call.</li>
      </ul>
    </li>
    <li>
      <strong>Custom Slash Commands</strong>
      <ul>
        <li>"Stored prompts," not system prompts. Good for repeatable actions/macros.</li>
      </ul>
    </li>
  </ul>
</CollapseDropdown>
...
<ul>
  <li>
    ...
    <ul>
      <li>
        <strong>User-level</strong> (reusable everywhere): <code>~/.claude/output-styles</code>
      </li>
      <li>
        <strong>Project-level</strong> (repo-scoped): <code>.claude/output-styles</code>
      </li>
    </ul>
  </li>
</ul>
...
- **Domain specialist modes (Custom)**
  ...
  - Capture checklists and communication style once; reuse across repos.
...
Tip: Combine a Performance Coach style with complexity-focused prompts to catch N+1 queries and needless copies early.
</CollapseDropdown>
...
- If style switches don't seem to persist: ensure `.claude/settings.local.json` is writable in the repo.  
  (This file is per-user and typically not committed; verify your `.gitignore` rules.)
