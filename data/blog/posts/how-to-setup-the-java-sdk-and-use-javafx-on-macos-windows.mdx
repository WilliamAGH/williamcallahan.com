...
<CollapseDropdown summary="6.2: Managing Multiple Java Versions">
  ...
  Official installers often put them in `/Library/Java/JavaVirtualMachines/` (system-wide).

  Tools like JetBrains IDEs (e.g., IDEA), Homebrew (`/opt/homebrew/opt/openjdk@21` on M-series Macs), or SDKMAN install them in the same path, but in your user directory ~/ with the same path otherwise.

  The `/usr/libexec/java_home` tool (which helps set `JAVA_HOME`) might pick a different JDK even if you *think* you've set your user one in your shell config. You can diagnose this with `java -version` and `/usr/libexec/java_home -V` to see which is active. If you find duplicates, removing the one(s) you don't need is usually the simplest fix.
...
<CollapseDropdown summary="6.5: IntelliJ IDEA's Maven Project Options">
