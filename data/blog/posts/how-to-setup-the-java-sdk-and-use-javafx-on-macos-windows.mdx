...
<BackgroundInfo title="Version Compatibility for JavaFX">
  Use the latest Long-Term Support (LTS) versions consistently. For example: - **JDK 21** - **JavaFX
  21** (or a version compatible with your JDK, e.g., JavaFX 22+ for JDK 21+) - **Scene Builder 23**
  (or latest corresponding LTS) Ensure your `pom.xml` or `build.gradle` reflects these versions. See{" "}
  <a href="#javafx-jdk-compatibility">Part 5.1</a> for a `pom.xml` example.
</BackgroundInfo>
...
<CollapseDropdown summary="2.1: Download and Install Scene Builder">
  Visit the{" "}
  <ExternalLink href="https://gluonhq.com/products/scene-builder/">
    Gluon Scene Builder website
  </ExternalLink>{" "}
...
<BackgroundInfo title="General Troubleshooting">
  - **Clean and Rebuild**: `./mvnw clean install` or `./gradlew clean build`. - **Check Logs**: Full
  error stack trace in IDE console. - **Simplify**: Create minimal reproducible example to isolate
  issues.
</BackgroundInfo>
