    ...
    <ShellTab label="Maven (OS Level)" isDefault={true}>
      <InstructionToggleTabs>
        <InstructionTab label="macOS (Homebrew)" isDefault={true}>
          ```bash brew install maven mvn -v # Verify ```
        </InstructionTab>
        <InstructionTab label="Linux (apt)">
          ```bash sudo apt update sudo apt install maven mvn -v # Verify ```
        </InstructionTab>
        <InstructionTab label="Manual Install (All OS)">
          ...
          ```bash # Example for macOS/Linux in ~/.zshrc or ~/.bashrc # export MAVEN_HOME=/opt/apache-maven-3.9.6 #
    ...
    <ShellTab label="Gradle (OS Level)">
      <InstructionToggleTabs>
        ...
        <InstructionTab label="Manual Install (All OS)">
          ...
          ```bash # Example for macOS/Linux in ~/.zshrc or ~/.bashrc # export GRADLE_HOME=/opt/gradle-8.7 # export
...
<ShellParentTabs>
    ...
    <CollapseDropdown summary="Project-Specific JAVA_HOME (Manual/Scripted)">
      <InstructionToggleTabs>
        <InstructionTab label="Temporary Override (Current Shell)" isDefault={true}>
          ```bash # Example: Point to a JDK 21 installation export JAVA_HOME=$(/usr/libexec/java_home -v 21) # Or
          directly: export JAVA_HOME=/Library/Java/JavaVirtualMachines/jdk-21.jdk/Contents/Home java -version ```
        </InstructionTab>
        <InstructionTab label="Project-Local Script">
          <div>`set_java_env.sh`:</div>
          ```bash #!/bin/bash export JAVA_HOME=$(/usr/libexec/java_home -v 17) # Or project's JDK path ```
        ...
        <InstructionTab label="Using direnv">
          ...
          ```bash # .envrc export JAVA_HOME=$(/usr/libexec/java_home -v 17) # Or specific path ```
  ...
  <ShellTab label="Alternative: SDKMAN! (CLI Tool)">
    <CollapseDropdown summary="Install & Use SDKMAN!">
      ```bash curl -s "https://get.sdkman.io" | bash source "$HOME/.sdkman/bin/sdkman-init.sh" sdk install java
      17.0.10-tem # Example install sdk use java 17.0.10-tem # Use in current shell ```
    </CollapseDropdown>
    <CollapseDropdown summary="Project Pinning (.sdkmanrc)">
      <div>In project directory:</div>
      ```bash sdk env init java 11.0.22-tem # Creates .sdkmanrc ```
      <div>SDKMAN! auto-switches Java version when entering the directory (after initial trust).</div>
      ```bash java -version # Shows .sdkmanrc version javac -version ```
    </CollapseDropdown>
  </ShellTab>
  <ShellTab label="Alternative: jenv (CLI Tool)">
    <CollapseDropdown summary="Install & Use jenv (macOS Homebrew example)">
      ```bash brew install jenv # Add to shell config (e.g., ~/.zshrc) echo 'export PATH="$HOME/.jenv/bin:$PATH"' >>
      ~/.zshrc echo 'eval "$(jenv init -)"' >> ~/.zshrc source ~/.zshrc jenv add /opt/homebrew/opt/openjdk@11 # Add
      installed JDK ```
    </CollapseDropdown>
    <CollapseDropdown summary="Project Pinning (.java-version)">
      <div>In project directory:</div>
      ```bash jenv local 17.0 # Creates .java-version ```
      <div>`jenv` auto-switches Java version.</div>
      ```bash java -version # Shows .java-version version ```
    </CollapseDropdown>
  </ShellTab>
</ShellParentTabs>
  ...
  <InstructionToggleTabs>
    <InstructionTab label="Maven (./mvnw)" isDefault={true}>
      <CollapseDropdown summary="Check for Updates">
        ...
        ```bash ./mvnw versions:display-dependency-updates ```
      </CollapseDropdown>
      <CollapseDropdown summary="Apply Updates">
        ```bash # Update specific dependency ./mvnw versions:use-latest-versions -Dincludes=groupId:artifactId # Or edit
        pom.xml manually, then: ./mvnw clean install ```
      </CollapseDropdown>
    </InstructionTab>
