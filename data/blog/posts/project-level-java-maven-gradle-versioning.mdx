...
<CollapseDropdown summary="Installing a Java Development Kit (JDK)">
  ...
  <ShellParentTabs>
      ...
      <InstructionToggleTabs>
        ...
        <InstructionTab label="Manual Download (Oracle/Adoptium)">
          <ol className="list-decimal list-inside space-y-2">
            <li>
              Download the macOS <code>.dmg</code> or <code>.tar.gz</code> installer from{" "}
              <ExternalLink href="https://www.oracle.com/java/technologies/downloads/">Oracle Java Downloads</ExternalLink>
              {" "}or{" "}
              <ExternalLink href="https://adoptium.net/">Adoptium (Eclipse Temurin)</ExternalLink>.
            </li>
            <li>
              Install using the <code>.dmg</code> or extract the <code>.tar.gz</code>, such as into{" "}
              <code>/Library/Java/JavaVirtualMachines/</code>.
            </li>
            <li>
              Set <code>JAVA_HOME</code> and <code>PATH</code> as shown in the Homebrew tab,
              adjusting the path to your installation.
            </li>
          </ol>
          <BackgroundInfo title="About Adoptium">
            Adoptium provides prebuilt, TCK-certified OpenJDK binaries under the Eclipse Temurin
            project. These are free to use, community-supported, and a popular choice for OpenJDK
            distributions.
          </BackgroundInfo>
        </InstructionTab>
      </InstructionToggleTabs>
    ...
    <ShellTab label="Windows">
      <ol>
        <li>
          Download the Windows <code>.exe</code> or <code>.msi</code> installer from{" "}
          <ExternalLink href="https://www.oracle.com/java/technologies/downloads/">Oracle</ExternalLink> or{" "}
          <ExternalLink href="https://adoptium.net/">Adoptium</ExternalLink>.
        </li>
        <li>
          Run the installer. Default location is often{" "}
          <code>{`C:\\Program Files\\Java\\jdk-21`}</code> or similar.
        </li>
        <li>
          Set Environment Variables:
          <ul>
            <li>Search for "environment variables" and select "Edit the system environment variables".</li>
            ...
            <li>
              ...
              <ul>
                ...
                <li>
                  Variable value: <code>{`C:\\Program Files\\Java\\jdk-21`}</code> (adjust to your
                  installation path)
                </li>
              </ul>
            </li>
            <li>Find the <code>Path</code> variable in "System variables", select it, and click "Edit...".</li>
            <li>
              Click "New" and add <code>{`%JAVA_HOME%\\bin`}</code>.
            </li>
...
<CollapseDropdown summary="Installing Build Tools (Maven & Gradle) - OS Level (Optional)">
  <div>
    While project wrappers (`mvnw`, `gradlew`) are preferred for project work, you might want global
    installations for bootstrapping new projects or for tools that expect them.
  </div>
  <ShellParentTabs>
    <ShellTab label="Maven (OS Level)" isDefault={true}>
      <InstructionToggleTabs>
        <InstructionTab label="macOS (Homebrew)" isDefault={true}>
          <pre>
            <code className="language-bash">{["brew install maven", "mvn -v # Verify"].join("\n")}</code>
          </pre>
        </InstructionTab>
        <InstructionTab label="Linux (apt)">
          <pre>
            <code className="language-bash">
              {["sudo apt update", "sudo apt install maven", "mvn -v # Verify"].join("\n")}
            </code>
          </pre>
        </InstructionTab>
        <InstructionTab label="Manual Install (All OS)">
          <div>
            ...
            <ExternalLink href="https://maven.apache.org/download.cgi">Apache Maven</ExternalLink>.
            2. Extract to a directory (e.g., `/opt/maven` on Linux/macOS, `C:\Program Files\Maven`
            on Windows). 3. Set `MAVEN_HOME` (or `M2_HOME`) environment variable to this directory.
            4. Add `MAVEN_HOME/bin` (or `M2_HOME/bin`) to your `PATH`. 5. Verify with `mvn -v`.
          </div>
          <pre>
            <code className="language-bash">
              {[
    ...
    <ShellTab label="Gradle (OS Level)">
      <InstructionToggleTabs>
        <InstructionTab label="macOS (Homebrew)" isDefault={true}>
          <pre>
            <code className="language-bash">{["brew install gradle", "gradle -v # Verify"].join("\n")}</code>
          </pre>
        </InstructionTab>
        <InstructionTab label="Linux (SDKMAN!)">
          <div>SDKMAN! (see Part 2) is excellent for managing global Gradle versions:</div>
          <pre>
            <code className="language-bash">{["sdk install gradle", "gradle -v # Verify"].join("\n")}</code>
          </pre>
        </InstructionTab>
        <InstructionTab label="Manual Install (All OS)">
          <div>
            1. Download Gradle binaries from{" "}
            <ExternalLink href="https://gradle.org/releases/">Gradle Releases</ExternalLink>. 2.
            Extract to a directory (e.g., `/opt/gradle` on Linux/macOS, `C:\Program Files\Gradle` on
            Windows). 3. Set `GRADLE_HOME` environment variable. 4. Add `GRADLE_HOME/bin` to your
            `PATH`. 5. Verify with `gradle -v`.
          </div>
          <pre>
            <code className="language-bash">
              {[
  ...
  <BackgroundInfo title="OS Install vs. Project Wrappers">
    Global (OS-level) installations of Maven/Gradle are for convenience or bootstrapping. **For
    actual project development and builds, always prefer using the project's Maven Wrapper
    (`./mvnw`) or Gradle Wrapper (`./gradlew`).** This ensures everyone uses the same build tool
    version, leading to reproducible builds. The wrappers will download the correct version if
    needed.
  </BackgroundInfo>
</CollapseDropdown>
...
<ShellParentTabs>
    ...
    <CollapseDropdown summary="Project-Specific JAVA_HOME (Manual/Scripted)">
      <InstructionToggleTabs>
        <InstructionTab label="Temporary Override (Current Shell)" isDefault={true}>
          <div>Example: Point to a JDK 21 installation.</div>
          <pre>
            <code className="language-bash">
              {[
        ...
        <InstructionTab label="Project-Local Script">
          ...
          <pre>
            <code className="language-bash">
              {["#!/bin/bash", "export JAVA_HOME=$(/usr/libexec/java_home -v 17) # Or project's JDK path"].join("\n")}
            </code>
          </pre>
        ...
        <InstructionTab label="Using direnv">
          ...
          <pre>
            <code className="language-bash">
              {["# .envrc", "export JAVA_HOME=$(/usr/libexec/java_home -v 17) # Or specific path"].join("\n")}
            </code>
          </pre>
  ...
  <ShellTab label="Alternative: SDKMAN! (CLI Tool)">
    ...
    <CollapseDropdown summary="Project Pinning (.sdkmanrc)">
      ...
      <pre>
        <code className="language-bash">{"sdk env init java 11.0.22-tem # Creates .sdkmanrc"}</code>
      </pre>
      <div>SDKMAN! auto-switches Java version when entering the directory (after initial trust).</div>
      <pre>
        <code className="language-bash">
          {["java -version # Shows .sdkmanrc version", "javac -version"].join("\n")}
        </code>
      </pre>
    </CollapseDropdown>
  </ShellTab>
  <ShellTab label="Alternative: jenv (CLI Tool)">
    ...
    <CollapseDropdown summary="Project Pinning (.java-version)">
      <div>In project directory:</div>
      <pre>
        <code className="language-bash">{"jenv local 17.0 # Creates .java-version"}</code>
      </pre>
      <div>`jenv` auto-switches Java version.</div>
      <pre>
        <code className="language-bash">{"java -version # Shows .java-version version"}</code>
      </pre>
    </CollapseDropdown>
  </ShellTab>
</ShellParentTabs>
...
<ShellParentTabs>
  <ShellTab label="Maven Wrapper (./mvnw) (Standard)" isDefault={true}>
    ...
    <CollapseDropdown summary="Adding or Updating the Wrapper">
      ...
      <pre>
        <code className="language-properties">{`# .mvn/wrapper/maven-wrapper.properties
    ...
    <CollapseDropdown summary="Core Maven Lifecycle & Commands (using ./mvnw)">
      ...
      <InstructionToggleTabs>
        ...
        <InstructionTab label="Dependency Management">
          ...
          <pre>
            <code className="language-xml">{`&lt;!-- pom.xml --&gt;
<dependencies>
    <dependency>
...
<ShellParentTabs>
  <ShellTab label="Gradle Wrapper (./gradlew) (Standard)" isDefault={true}>
    ...
    <CollapseDropdown summary="Adding or Configuring the Wrapper">
      ...
      <pre>
        <code className="language-bash">
          {["# In project root", "gradle wrapper --gradle-version 8.7 # Specify desired Gradle version"].join("\n")}
        </code>
      </pre>
      <div>The wrapper configuration, including the Gradle distribution URL, is in `gradle/wrapper/gradle-wrapper.properties`.</div>
      <pre>
        <code className="language-properties">{`# gradle/wrapper/gradle-wrapper.properties
    ...
    <CollapseDropdown summary="Core Gradle Tasks & Commands (using ./gradlew)">
      ...
      <InstructionToggleTabs>
        ...
        <InstructionTab label="Dependency Management">
          ...
          <pre>
            <code className="language-kotlin">{`// build.gradle.kts
dependencies {
    ...
    <CollapseDropdown summary="Gradle Wrapper & Java Version">
      ...
      <pre>
        <code className="language-groovy">{`// build.gradle (Groovy DSL)
java {
  ...
  <InstructionToggleTabs>
    <InstructionTab label="Maven (./mvnw)" isDefault={true}>
      <CollapseDropdown summary="Check for Updates">
        ...
        <pre>
          <code className="language-bash">{"./mvnw versions:display-dependency-updates"}</code>
        </pre>
      </CollapseDropdown>
    ...
    <InstructionTab label="Gradle (./gradlew)">
      <CollapseDropdown summary="Check for Updates (with plugin)">
        ...
        <pre>
          <code className="language-bash">{"./gradlew dependencyUpdates"}</code>
        </pre>
      </CollapseDropdown>
      <CollapseDropdown summary="Apply Updates">
        <div>Edit `build.gradle(.kts)` versions manually, then:</div>
        <pre>
          <code className="language-bash">{"./gradlew build"}</code>
        </pre>
      </CollapseDropdown>
    </InstructionTab>
  </InstructionToggleTabs>
  <BackgroundInfo title="IDE Support">
    IntelliJ IDEA and VS Code (with Java extensions) often provide UI features to detect and update
    dependencies.
  </BackgroundInfo>
</CollapseDropdown>
