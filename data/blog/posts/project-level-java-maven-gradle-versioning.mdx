    ...
    <ShellTab label="Maven (OS Level)" isDefault={true}>
      <InstructionToggleTabs>
        ...
        <InstructionTab label="Manual Install (All OS)">
          ...
          ```bash # Example for macOS/Linux in ~/.zshrc or ~/.bashrc # export
          MAVEN_HOME=/opt/apache-maven-3.9.6 # export PATH="$MAVEN_HOME/bin:$PATH" ```
        </InstructionTab>
      </InstructionToggleTabs>
    </ShellTab>
    <ShellTab label="Gradle (OS Level)">
      <InstructionToggleTabs>
        ...
        <InstructionTab label="Manual Install (All OS)">
          ...
          ```bash # Example for macOS/Linux in ~/.zshrc or ~/.bashrc # export
          GRADLE_HOME=/opt/gradle-8.7 # export PATH="$GRADLE_HOME/bin:$PATH" ```
        </InstructionTab>
      </InstructionToggleTabs>
    </ShellTab>
  </ShellParentTabs>
  ...
  <InstructionToggleTabs>
    <InstructionTab label="Maven (./mvnw)" isDefault={true}>
      ...
      <CollapseDropdown summary="Apply Updates">
        ```bash # Update specific dependency ./mvnw versions:use-latest-versions
        -Dincludes=groupId:artifactId # Or edit pom.xml manually, then: ./mvnw clean install ```
      </CollapseDropdown>
    </InstructionTab>
