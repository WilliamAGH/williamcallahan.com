...
export class FailureTracker<T> {
   ...
   */
  getStats() {
    const permanent = Array.from(this.failures.values())
      .filter(f => f.permanentFailure).length;
  ...
  private pruneOldest(): void {
    ...
    const sorted = Array.from(this.failures.entries())
    ...
    debugLog(
      `[${this.config.name}] Pruned ${toRemove} oldest entries`,
      'warn'
    );
  }
}