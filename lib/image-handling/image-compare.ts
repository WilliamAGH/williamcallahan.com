...
function generateStructuralHash(meta: BasicImageMeta, buffer: Buffer): string {
  const sizeBucket = 
    ...
    meta.width < 32 ? 'tiny' :
    meta.width < 64 ? 'small' :
    meta.width < 128 ? 'medium' : 'large';
  
  const aspect = meta.width && meta.height ? meta.width / meta.height : 1;
  const aspectBucket = 
    aspect < 0.9 ? 'tall' :
    aspect > 1.1 ? 'wide' : 'square';
  
  const compressionRatio = meta.width && meta.height ? 
    buffer.length / (meta.width * meta.height) : 0;
  const compressionBucket = 
    compressionRatio < 0.1 ? 'high' :
    compressionRatio < 0.5 ? 'medium' : 'low';
...
export function compareSignatures(sig1: ImageSignature, sig2: ImageSignature): number {
  ...
  if (sig1.dimensions.width > 0 && sig2.dimensions.width > 0) {
    const dimSim = 1 - Math.abs(sig1.dimensions.width - sig2.dimensions.width) / 
  ...
  const sizeSim = 1 - Math.abs(sig1.fileSize - sig2.fileSize) / 
                  Math.max(sig1.fileSize, sig2.fileSize);
