...
export async function processBookmarksInBatches(
  ...
  console.log(`${LOG_PREFIX} Starting OpenGraph enrichment for ${bookmarks.length} bookmarks${useBatchMode ? ' (batch mode)' : ''}`);
    ...
    try {
      ...
      if ((useBatchMode || isDev) && i % 10 === 0 && i > 0) {
        ...
        if (useBatchMode) {
          ...
          console.log(`${LOG_PREFIX}   - Images scheduled for background persistence: ${imageStats.imagesNewlyPersisted}`);
          console.log(`${LOG_PREFIX}   - Images already stored: ${imageStats.imagesAlreadyInS3}`);
        }
      ...
      if (ogData && !ogData.error && ogData.ogMetadata) {
        ...
        if (result.success) {
                  ...
                  try {
                    ...
                    if (assetResponse.ok) {
                      ...
                      if (result.s3Url) {
                        ...
                        console.warn(`${LOG_PREFIX} ⚠️ Failed to persist Karakeep asset to S3, using proxy: ${finalImage}`);
                      }
                    } else {
                      ...
                      imageStats.bookmarksUsingKarakeepImage++;
                      imageStats.karakeepFallbackDetails.push({ url: bookmark.url, karakeepImage: finalImage });
                      console.warn(`${LOG_PREFIX} ⚠️ Failed to fetch Karakeep asset (${assetResponse.status}), using proxy: ${finalImage}`);
                    }
                  } catch (error) {
                    bookmark.ogImage = finalImage;
                    imageStats.bookmarksUsingKarakeepImage++;
          ...
          console.error(`${LOG_PREFIX} ❌ Failed to parse OpenGraph metadata for ${bookmark.url}: ${result.error.message}`);
                ...
                try {
                  ...
                  if (assetResponse.ok) {
                    ...
                    if (result.s3Url) {
                      ...
                      imageStats.bookmarksUsingKarakeepImage++;
                      imageStats.karakeepFallbackDetails.push({ url: bookmark.url, karakeepImage });
                      console.warn(`${LOG_PREFIX} ⚠️ Failed to persist Karakeep fallback asset to S3, using proxy: ${karakeepImage}`);
                    }
                  } else {
                    // Fallback to proxy URL if fetch failed
                    bookmark.ogImage = karakeepImage;
                    imageStats.bookmarksUsingKarakeepImage++;
                    imageStats.karakeepFallbackDetails.push({ url: bookmark.url, karakeepImage });
                    console.warn(`${LOG_PREFIX} ⚠️ Failed to fetch Karakeep fallback asset (${assetResponse.status}), using proxy: ${karakeepImage}`);
                  }
                } catch (error) {
                  bookmark.ogImage = karakeepImage;
                  imageStats.bookmarksUsingKarakeepImage++;
                  imageStats.karakeepFallbackDetails.push({ url: bookmark.url, karakeepImage });
                  console.error(`${LOG_PREFIX} ❌ Error fetching Karakeep fallback asset for bookmark ${bookmark.url}:`);
                  console.error(`${LOG_PREFIX}   Asset ID: ${assetId}`);
                  console.error(`${LOG_PREFIX}   Error: ${error instanceof Error ? error.message : String(error)}`);
                  imageStats.errorDetails.push({ 
                    url: bookmark.url, 
  ...
  if (useBatchMode) {
    ...
    console.log(`${LOG_PREFIX}   📋 Images scheduled for background S3 persistence: ${imageStats.imagesNewlyPersisted}`);
    console.log(`${LOG_PREFIX}   📦 Images already stored in S3: ${imageStats.imagesAlreadyInS3}`);
  }
  console.log(`${LOG_PREFIX}`);
  console.log(`${LOG_PREFIX} PROCESSING STATS:`);
  const totalWithImages = imageStats.bookmarksUsingS3Image + imageStats.bookmarksUsingOpenGraphImage + imageStats.bookmarksUsingKarakeepImage;
  console.log(`${LOG_PREFIX}   Total with images: ${totalWithImages} (${Math.round((totalWithImages / imageStats.totalBookmarks) * 100)}%)`);
  console.log(`${LOG_PREFIX}   Processing errors: ${imageStats.bookmarksWithErrors}`);
  ...
  if (imageStats.imagesNewlyPersisted === 0) {
    if (imageStats.bookmarksUsingS3Image === totalWithImages) {
      ...
      console.warn(`${LOG_PREFIX} ⚠️ WARNING: ${imageStats.bookmarksUsingKarakeepImage} bookmarks are using Karakeep proxy URLs`);
      ...
      console.error(`${LOG_PREFIX} ❌ ERROR: ${imageStats.bookmarksUsingKarakeepImage} bookmarks failed to persist Karakeep images to S3`);
