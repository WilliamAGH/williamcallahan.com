...
import type {
  ...
  GraphQLContributionWeek,
  GraphQLContributionDay as ImportedGraphQLContributionDay
...
export async function refreshGitHubActivityDataFromApi(): Promise<{trailingYearData: StoredGithubActivityS3, allTimeData: StoredGithubActivityS3} | null> {
  ...
  try {
    ...
    console.error("[DataAccess/GitHub] CRITICAL: Failed to fetch repository list via GraphQL:", gqlError instanceof Error ? gqlError.message : String(gqlError));
    return null;
  }
  ...
  if (uniqueRepoArray.length === 0) {
    ...
    const emptyRawResponse: StoredGithubActivityS3 = {
      source: 'api', data: [], totalContributions: 0, linesAdded: 0, linesRemoved: 0, dataComplete: true, allTimeTotalContributions: 0,
    };
    ...
    const batchPromises = batch.map(async (repo) => {
      ...
      try {
        ...
        if (statsResponse.status === 202) {
          ...
          if (VERBOSE) console.log(`[DataAccess/GitHub] Trailing Year: GitHub API returned 202 (pending) for ${repoOwnerLogin}/${repoName}.`);
          ...
          repoDataCompleteForYear = false;
          if (VERBOSE) console.warn(`[DataAccess/GitHub] Trailing Year: Error fetching stats for ${repoOwnerLogin}/${repoName}. Status: ${statsResponse.status}.`);
        }
        if (apiStatus === 'complete' && userWeeklyStatsFromApi.length > 0) {
          ...
          if (existingDataBuffer && existingDataBuffer.length > 0) {
            if (VERBOSE) console.log(`[DataAccess/GitHub-S3] Trailing Year: Using existing S3 CSV data for ${repoOwnerLogin}/${repoName} due to API status: ${apiStatus}`);
        ...
        if (finalStatsToSaveForRepo.length > 0 && (apiStatus === 'complete' || apiStatus === 'empty_no_user_contribs' || (apiStatus !== 'fetch_error' && apiStatus !== 'pending_202_from_api' && finalStatsToSaveForRepo.length >0) )) {
           await writeBinaryS3(repoStatS3Key, Buffer.from(finalStatsToSaveForRepo.map(w => `${w.w},${w.a},${w.d},${w.c}`).join('\n')), 'text/csv');
           if (VERBOSE) console.log(`[DataAccess/GitHub-S3] Trailing Year: CSV for ${repoOwnerLogin}/${repoName} updated/written. Weeks: ${finalStatsToSaveForRepo.length}. API Status: ${apiStatus}`);
        } else if (finalStatsToSaveForRepo.length === 0 && (apiStatus === 'pending_202_from_api' || apiStatus === 'fetch_error')) {
            console.warn(`[DataAccess/GitHub-S3] Trailing Year: No stats data to save for ${repoOwnerLogin}/${repoName} (API status: ${apiStatus}, no usable existing S3 data). CSV not written.`);
            repoDataCompleteForYear = false;
        }
        for (const week of finalStatsToSaveForRepo) {
          ...
          console.warn(`[DataAccess/GitHub] Trailing Year: Critical error processing stats for ${repoOwnerLogin}/${repoName}:`, repoError instanceof Error ? repoError.message : String(repoError));
          repoDataCompleteForYear = false;
      ...
      if (thisRepoAllTimeDataContributed) {
        ...
        allTimeCategoryStats[categoryKey].netChange = (allTimeCategoryStats[categoryKey].netChange || 0) + (currentRepoAllTimeLinesAdded - currentRepoAllTimeLinesRemoved);
  ...
  try {
    ...
    if (gqlResponse?.user?.contributionsCollection?.contributionCalendar) {
      ...
      calendar.weeks.forEach((week: GraphQLContributionWeek) => {
        week.contributionDays.forEach((day: ImportedGraphQLContributionDay) => {
      ...
      console.log(`[DataAccess/GitHub] Successfully fetched contribution calendar. Total contributions (trailing year): ${yearTotalCommits}`);
    } else {
      console.warn('[DataAccess/GitHub] Failed to fetch or parse GraphQL contribution calendar. Calendar data will be empty.');
      // Keep yearTotalCommits as 0 and trailingYearContributionsCalendar as empty
    }
  } catch (gqlError: unknown) {
    console.error("[DataAccess/GitHub] CRITICAL: Error fetching GraphQL contribution calendar:", gqlError instanceof Error ? gqlError.message : String(gqlError));
  ...
  try {
    ...
    const yearSummaryData: GitHubActivitySummary = {
      ...
      totalContributions: trailingYearData.totalContributions, totalLinesAdded: trailingYearData.linesAdded || 0, totalLinesRemoved: trailingYearData.linesRemoved || 0, netLinesOfCode: netYearLoc, dataComplete: trailingYearData.dataComplete !== undefined ? trailingYearData.dataComplete : true, totalRepositoriesContributedTo: uniqueRepoArray.length, linesOfCodeByCategory: yearCategoryStats,
    };
    await writeJsonS3(summaryS3Key, yearSummaryData);
    if (VERBOSE) console.log(`[DataAccess/GitHub-S3] Trailing year GitHub summary saved to ${summaryS3Key}`);
  } catch (summaryError: unknown) {
     console.error('[DataAccess/GitHub-S3] Failed to write trailing year GitHub summary:', summaryError instanceof Error ? summaryError.message : String(summaryError));
  }

  console.log(`[DataAccess/GitHub] All-time stats calculated in-memory: Commits=${allTimeTotalCommits}, Added=${allTimeLinesAdded}, Removed=${allTimeLinesRemoved}, Complete=${allTimeOverallDataComplete}`);

  // Consistency checks for all-time vs. trailing year
  if (allTimeTotalCommits < yearTotalCommits) {
    console.warn(`[DataAccess/GitHub] Critical inconsistency detected: All-time commits (${allTimeTotalCommits}) less than trailing year commits (${yearTotalCommits}). This may indicate an issue with historical data aggregation if all-time is genuinely lower than a single year's recent activity.`);
  ...
  if (allTimeLinesAdded < yearLinesAdded) {
    console.warn(`[DataAccess/GitHub] Inconsistency detected: All-time lines added (${allTimeLinesAdded}) less than trailing year (${yearLinesAdded}). This might be valid if historical data had net negative contributions over time but recent year was very positive. Verifying logic.`);
  ...
  if (allTimeLinesRemoved < yearLinesRemoved) {
    console.warn(`[DataAccess/GitHub] Inconsistency detected: All-time lines removed (${allTimeLinesRemoved}) less than trailing year (${yearLinesRemoved}). Similar to lines added, this could be valid. Verifying logic.`);
    // Not automatically correcting.
  }

  let reconciledAllTimeTotalCommits = allTimeTotalCommits;
  if (allTimeTotalCommits < yearTotalCommits) {
    console.log(`[DataAccess/GitHub] Reconciling totalContributions for all-time data: Calculated all-time commits (${allTimeTotalCommits}) is less than trailing year commits (${yearTotalCommits}). Using trailing year count as the floor for all-time total contributions.`);
  ...
  try {
    ...
    const finalAllTimeCategoryStats: GitHubActivitySummary['linesOfCodeByCategory'] = { frontend: { linesAdded: 0, linesRemoved: 0, netChange: 0, repoCount: 0 }, backend: { linesAdded: 0, linesRemoved: 0, netChange: 0, repoCount: 0 }, dataEngineer: { linesAdded: 0, linesRemoved: 0, netChange: 0, repoCount: 0 }, other: { linesAdded: 0, linesRemoved: 0, netChange: 0, repoCount: 0 } };
    ...
    Object.keys(allTimeCategoryStats).forEach(catKey => {
    ...
    if (VERBOSE) console.log(`[DataAccess/GitHub-S3] All-time GitHub summary saved to ${ALL_TIME_SUMMARY_S3_KEY_FILE}`);
  } catch (summaryError: unknown) {
    console.error('[DataAccess/GitHub-S3] Failed to write all-time GitHub summary:', summaryError instanceof Error ? summaryError.message : String(summaryError));
  }
  ...
  const emptyStoredBase: Omit<StoredGithubActivityS3, 'source' | 'error' | 'details'> = {
    data: [], totalContributions: 0, linesAdded: 0, linesRemoved: 0, dataComplete: false, allTimeTotalContributions: 0,
  };
  const safeTrailingYearData: StoredGithubActivityS3 = trailingYearData || {
    ...emptyStoredBase, source: 'api', error: "Trailing year data generation failed or was incomplete during refresh", dataComplete: false,
  };
  const safeAllTimeData: StoredGithubActivityS3 = allTimeData || {
    ...emptyStoredBase, source: 'api', error: "All-time data generation failed or was incomplete during refresh", dataComplete: false,
  };
  ...
  try {
    ...
    if (VERBOSE) console.log(`[DataAccess/GitHub-S3] Combined GitHub activity data (ApiResponse structure) saved to ${GITHUB_ACTIVITY_S3_KEY_FILE}`);
  } catch (error: unknown) {
    console.error(`[DataAccess/GitHub-S3] Failed to write combined GitHub activity data to S3:`, error instanceof Error ? error.message : String(error));
  }
...
export async function getGithubActivity(): Promise<UserActivityView> {
  ...
  if (cachedData) {
    if (VERBOSE) console.log('[DataAccess/GitHub:getGithubActivity] Returning GitHub activity from in-memory cache.');
    return cachedData;
  }

  if (VERBOSE) console.log(`[DataAccess/GitHub:getGithubActivity] Attempting to read GitHub activity from S3: ${GITHUB_ACTIVITY_S3_KEY_FILE}`);
  ...
  if (s3ActivityData?.trailingYearData && s3ActivityData.cumulativeAllTimeData) {
    if (VERBOSE) console.log('[DataAccess/GitHub:getGithubActivity] Successfully read and parsed GitHub activity from S3 (expected nested structure).');
    ...
    if (allTimeContributions < trailingYearContributions) {
      console.warn(`[DataAccess/GitHub:getGithubActivity] Data inconsistency: All-time contributions (${allTimeContributions}) < trailing year (${trailingYearContributions}). Displaying S3-derived all-time count; trailing year count from API is higher.`);
    }
    ...
    const userView: UserActivityView = {
      ...
      allTimeStats: {
        ...
        linesAdded: Math.max(s3ActivityData.cumulativeAllTimeData.linesAdded || 0, s3ActivityData.trailingYearData.linesAdded || 0),
        linesRemoved: Math.max(s3ActivityData.cumulativeAllTimeData.linesRemoved || 0, s3ActivityData.trailingYearData.linesRemoved || 0),
      },
    ...
    if (isOldFlatStoredGithubActivityS3Format(s3ActivityData)) {
      console.warn('[DataAccess/GitHub:getGithubActivity] Detected outdated flat structure in S3. Attempting partial data recovery.');
      ...
      errorView = {
        source: 'error',
        error: 'Outdated data format detected. Some data may be incomplete. A refresh is recommended.',
        trailingYearData: {
      ...
      console.warn(`[DataAccess/GitHub:getGithubActivity] S3 data has invalid structure. Attempting partial data recovery.`);
      ...
      console.warn(`[DataAccess/GitHub:getGithubActivity] Failed to read GitHub activity from S3. Returning default values.`);
      errorView = {
    ...
    try {
      ...
      if (!csvBuffer || csvBuffer.length === 0) {
        ...
        if (lines.length === 0) {
          ...
          const hasInvalidLines = lines.some(line => {
            ...
            return parts.length !== 4 || parts.some(part => isNaN(Number(part)));
          });
          if (hasInvalidLines) {
            console.log(`[DataAccess/GitHub] CSV repair: Invalid data format in CSV for ${repoOwner}/${repoName}`);
            needsRepair = true;
          }
        }
      }
      ...
      if (needsRepair) {
        ...
        const statsResponse = await fetchWithRetry(
          ...
          { headers: { 'Authorization': `Bearer ${GITHUB_API_TOKEN}`, 'Accept': 'application/vnd.github.v3+json' } }
        );
        if (statsResponse.ok) {
          const contributors = await statsResponse.json() as GithubContributorStatsEntry[];
          const ownerLoginLower = GITHUB_REPO_OWNER.toLowerCase();
          ...
          if (userStatsEntry && userStatsEntry.weeks && Array.isArray(userStatsEntry.weeks)) {
            ...
            if (weeklyStats.length > 0) {
              ...
              console.log(`[DataAccess/GitHub] CSV repair: Successfully repaired ${repoOwner}/${repoName} with ${weeklyStats.length} weeks of data`);
              repairedCount++;
            } else {
              console.warn(`[DataAccess/GitHub] CSV repair: No weekly stats found for ${repoOwner}/${repoName} from API to repair.`);
              failedCount++; // Or consider if this is a success if the API says no data
            }
          } else {
            console.warn(`[DataAccess/GitHub] CSV repair: No user-specific stats found for ${repoOwner}/${repoName} from API.`);
            failedCount++;
          }
        } else {
          console.warn(`[DataAccess/GitHub] CSV repair: Failed to fetch stats for ${repoOwner}/${repoName}, status ${statsResponse.status}`);
          failedCount++;
        }
      }
    } catch (error) {
      console.error(`[DataAccess/GitHub] CSV repair: Error processing ${repoOwner}/${repoName}:`, error);
      failedCount++;
    }
  }
  console.log(`[DataAccess/GitHub] CSV repair completed: Scanned ${repoList.length} repos, repaired ${repairedCount}, failed ${failedCount}`);
  return { scannedRepos: repoList.length, repairedRepos: repairedCount, failedRepairs: failedCount };
}
...
export async function calculateAndStoreAggregatedWeeklyActivity(): Promise<{ aggregatedActivity: AggregatedWeeklyActivity[], overallDataComplete: boolean } | null> {
  if (process.env.DRY_RUN === 'true') {
    if (VERBOSE) console.log('[DataAccess/GitHub-S3] DRY RUN mode: skipping aggregated weekly activity calculation.');
    return null;
  }
  ...
  try {
    ...
    if (VERBOSE) console.log(`[DataAccess/GitHub-S3] Found ${s3StatFileKeys.length} potential stat files in S3.`);
  } catch (listError: unknown) {
    const message = listError instanceof Error ? listError.message : String(listError);
    console.error(`[DataAccess/GitHub-S3] Aggregation: Error listing S3 objects in ${REPO_RAW_WEEKLY_STATS_S3_KEY_DIR}/:`, message);
  ...
  if (s3StatFileKeys.length === 0) {
    if (VERBOSE) console.log(`[DataAccess/GitHub-S3] Aggregation: No raw weekly stat files found in S3 path ${REPO_RAW_WEEKLY_STATS_S3_KEY_DIR}/. Nothing to aggregate.`);
  ...
  for (const repoStatS3Key of s3StatFileKeys) {
    try {
      ...
      if (!buf) {
        if (VERBOSE) console.warn(`[DataAccess/GitHub-S3] Aggregation: No data in ${repoStatS3Key}, skipping.`);
      ...
      if (VERBOSE) console.warn(`[DataAccess/GitHub-S3] Aggregation: Error reading ${repoStatS3Key}, skipping.`, err);
      overallDataComplete = false; // Error reading a file means overall is not complete
      continue;
    }
  }
  if (VERBOSE) console.log(`[DataAccess/GitHub-S3] Aggregation: Processed ${s3StatFileKeys.length} S3 stat files from ${REPO_RAW_WEEKLY_STATS_S3_KEY_DIR}.`);
  ...
  console.log(`[DataAccess/GitHub-S3] Aggregated weekly activity calculated and stored to ${AGGREGATED_WEEKLY_ACTIVITY_S3_KEY_FILE}. Total weeks aggregated: ${aggregatedActivity.length}. Overall data complete: ${overallDataComplete}`);
