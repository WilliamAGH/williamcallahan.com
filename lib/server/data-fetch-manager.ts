...
import { SEARCH_S3_PATHS, IMAGE_MANIFEST_S3_PATHS, IMAGE_S3_PATHS, CONTENT_GRAPH_S3_PATHS } from "@/lib/constants";
...
export class DataFetchManager {
  ...
  /**
   * Build content graph with pre-computed relationships
   * This pre-computes all similarity scores, slug mappings, and tag relationships
   * to ensure hydration safety and optimal performance.
  ...
  private async buildContentGraph(config: DataFetchConfig): Promise<DataFetchOperationSummary> {
    ...
    try {
      ...
      const tagGraph = this.buildTagGraph(allContent);
  ...
  /**
   * Build tag co-occurrence graph from content
   */
  private buildTagGraph(allContent: Array<{ type: string; id: string; tags?: string[] }>): { 
    tags: Record<string, {
      count: number;
    ...
    for (const content of allContent) {
      ...
      for (const tag of content.tags || []) {
        ...
        for (const otherTag of content.tags || []) {
