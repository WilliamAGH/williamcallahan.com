...
const STOP_WORDS = new Set([
  ...
  "to", "was", "will", "with", "the", "this", "these", "those", "what",
  "when", "where", "which", "who", "why", "how", "all", "would", "should",
  "could", "can", "may", "might", "must", "shall", "will", "need", "use",
  "used", "using", "make", "makes", "made", "get", "gets", "got", "getting",
  "we", "you", "they", "our", "your", "their", "my", "his", "her", "its",
...
const TECHNICAL_TERMS = new Set([
  "ai", "ml", "api", "ui", "ux", "sdk", "cli", "gui", "ide", "cdn",
  "sql", "orm", "jwt", "ssl", "tls", "http", "https", "rest", "graphql",
  "ci", "cd", "k8s", "s3", "ec2", "vpc", "dns", "ip", "tcp", "udp",
  "css", "html", "jsx", "tsx", "xml", "json", "yaml", "toml",
  "b2b", "b2c", "saas", "paas", "iaas", "crm", "erp", "hr", "bi",
  "vc", "pe", "ipo", "m&a", "roi", "arr", "mrr", "cac", "ltv"
]);
...
const DOMAIN_KEYWORDS = new Set([
  ...
  "rust", "ruby", "php", "c++", "c#", "scala", "elixir", "haskell",
  
  // Frameworks
  "react", "vue", "angular", "svelte", "nextjs", "nuxt", "gatsby",
  "express", "fastify", "nestjs", "django", "flask", "rails", "spring",
  "laravel", "symfony", "phoenix", "gin", "echo", "fiber",
  
  // Databases
  "postgresql", "mysql", "mongodb", "redis", "elasticsearch", "cassandra",
  "dynamodb", "firestore", "supabase", "prisma", "typeorm", "sequelize",
  
  // Cloud/DevOps
  "docker", "kubernetes", "terraform", "ansible", "jenkins", "github",
  "gitlab", "bitbucket", "aws", "azure", "gcp", "vercel", "netlify",
  
  // Business/Finance
  "startup", "investment", "funding", "revenue", "growth", "market",
  "platform", "marketplace", "enterprise", "analytics", "automation",
  
  // AI/ML
  "machine", "learning", "neural", "network", "model", "training",
  "inference", "embedding", "vector", "transformer", "llm", "chatbot"
]);
...
function extractBigrams(text: string): string[] {
  ...
  for (let i = 0; i < words.length - 1; i++) {
    ...
    const technicalPhrases = [
      "machine learning", "artificial intelligence", "venture capital",
      "series a", "series b", "web app", "mobile app", "open source",
      "real time", "data science", "big data", "cloud computing",
      "digital transformation", "user experience", "customer service"
    ];
...
export function extractKeywords(
  ...
  const existingTagsNormalized = new Set(
    existingTags.map(tag => tag.toLowerCase().trim())
  );
...
export function extractCrossContentKeywords(
  ...
  const techMatches = (title + ' ' + description).toLowerCase().match(
    /\b(react|vue|angular|nextjs|django|rails|spring|nodejs|python|java|typescript|javascript|golang|rust)\b/gi
  );
  if (techMatches) {
  ...
  const businessMatches = (title + ' ' + description).toLowerCase().match(
...
export function hasInvestmentContext(text: string): boolean {
  const investmentTerms = [
    "invest", "funding", "venture", "capital", "series", "seed",
    "startup", "portfolio", "equity", "valuation", "raise", "round"
  ];
  
  const lowercased = text.toLowerCase();
  return investmentTerms.some(term => lowercased.includes(term));
}
...
export function hasTechnicalContext(text: string): boolean {
  const techTerms = [
    "code", "programming", "developer", "software", "api", "framework",
    "library", "database", "server", "frontend", "backend", "fullstack"
  ];
