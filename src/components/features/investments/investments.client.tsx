      ...
      <div className={cn("p-6", isMaximized ? "overflow-y-auto flex-grow" : "")}>
        ...
        <>
          <div className="prose dark:prose-invert max-w-none mb-8">
            <p>
              Below is the data (periodically updated) on all of my private startup investments
              to-date. The idea is to be transparent about all of them: no{" "}
              ...
              and no{" "}
              <ExternalLink href="https://en.wikipedia.org/wiki/Survivorship_bias">
                survivorship bias
              </ExternalLink>
              . I&apos;ve sought in good faith to represent them all here.
            </p>
            <p>
              The vast majority of these investments were passive / on the sidelines. That means I
              wasn&apos;t a large or meaningful part of their fundraising round, just allowed to
              participate with a smaller amount (typically $5,000 to $25,000).
            </p>
            <p>
              The longer-term goal of this dataset is to shed light on early stage investing to make
              it better for everyone involved. To bring some of the rigor of public markets with the{" "}
            ...
            <details className="mt-8 group">
              ...
              <ol className="list-decimal list-outside space-y-4 ml-4 mt-4">
                <li className="space-y-2">
                  <p className="italic">
                    Return calculations require assigning at least two valuations: when you invest
                    and at some later point, either when you exit or at an arbitrary point in time.
                    Exits are easy to calculate (the price is set by someone else, just as the
                    investment had), but the majority of current investments otherwise must be
                    valued arbitrarily by some estimate of current value.
                  </p>
                  <p className="italic">
                    Currently I am using the simplest methodology available: the last fundraising
                    round valuation. That means if they haven&apos;t raised more money since, the
                    valuation is statistically more likely to be overstated than understated.
                    Remember, this is not an endorsement or investment advice, nor an offer for any
                    securities.
                  </p>
                </li>
                <li className="space-y-2">
                  <p className="italic">
                    A lot of the returns show small declines or near complete losses but not total
                    losses. These should probably stand out as peculiar, but there&apos;s a good
                    reason for both: fees/transaction costs.
                  </p>
                  <p className="italic">
                    The vast majority of deals have some allocation of transaction costs for legal
                    and related expenses incurred in the transaction. That means less than 100% of
                    the principal ends up getting invested in the company. Therefore it immediately
                    has a negative return equal to the amount of transaction expense incurred. And
                    in the event of a total loss without a distribution, it has been accounted for
                    as a total loss of the principal invested excluding the transaction costs.
                  </p>
                </li>
              </ol>
            </details>
          ...
          <div className="space-y-6">
            {investments.map((investment) => (
              <InvestmentCardClient
                key={`${investment.id}-${investment.invested_year}`}
                {...investment}
              />
            ))}
          </div>
