...
/**
 * Get placeholder image URL, falling back to local path if S3 is unavailable.
 *
 * **Design rationale**: Placeholder images are bundled locally specifically so
 * builds always work offline. Falling back to local is correct behavior, not an error.
 * The "OrLocal" suffix makes this fallback behavior explicit to callers per RC1.
 *
 * @param localPath - Local path to the placeholder image (e.g., "/images/company-placeholder.svg")
 * @returns CDN URL if available, otherwise the local path (both are valid, working URLs)
 */
export function getPlaceholderImageUrlOrLocal(localPath: string): string {
  try {
    // Try to get S3 URL first for CDN benefits
    return getStaticImageUrl(localPath);
  } catch (error) {
    // Fallback to local path - this is expected behavior for placeholders
    if (!isTestEnvironment) {
      console.warn(`[PlaceholderImages] Using local fallback: ${localPath}`, error);
    }
    return localPath;
  }
}
...
/**
 * Get the company placeholder image URL (CDN if available, local otherwise)
 */
export function getCompanyPlaceholder(): string {
  return getPlaceholderImageUrlOrLocal(PLACEHOLDER_IMAGES.COMPANY_PLACEHOLDER);
}
