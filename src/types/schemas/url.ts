...
const IPV6_EMBEDDED_IPV4_PREFIXES = ["::ffff:", "0:0:0:0:0:ffff:", "::", "0:0:0:0:0:0:"];

/**
 * Extract the IPv4 address from an IPv6-mapped or IPv4-compatible address.
 *
 * @example
 * extractEmbeddedIPv4("::ffff:127.0.0.1") // "127.0.0.1"
 * extractEmbeddedIPv4("::7f00:1")          // "127.0.0.1"
 */
function extractEmbeddedIPv4(hostname: string): string | null {
  const lower = hostname.toLowerCase();
  const prefix = IPV6_EMBEDDED_IPV4_PREFIXES.find((candidate) => lower.startsWith(candidate));
  if (!prefix) {
    return null;
  }
...
function isPrivateIPv4(hostname: string): boolean {
  ...
  if (a === 100 && b >= 64 && b <= 127) return true; // 100.64.0.0/10 (RFC 6598)
...
export function isPrivateIP(hostname: string): boolean {
  ...
  const embeddedIpv4 = extractEmbeddedIPv4(cleanHostname);
