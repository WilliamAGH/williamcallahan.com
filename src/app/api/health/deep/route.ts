...
async function checkBookmarks(): Promise<DeepCheckResult> {
  ...
  const { result: mapping, check: mappingCheck } = await measure("Bookmarks: Load Slug Mapping", async () => {
  ...
  const { check: validationCheck } = await measure("Bookmarks: Validate First Bookmark", async () => {
...
export async function GET() {
  ...
  const { result: bookmarksResult, check: wrapperCheck } = await measure("Bookmarks Critical Path", checkBookmarks);
