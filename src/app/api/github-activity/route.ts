 ...
 *
 * Provides client-side access to PostgreSQL-backed GitHub activity data
...
export async function GET(request: NextRequest) {
  ...
  // because PostgreSQL-backed refreshes are handled by POST /api/github-activity/refresh
  ...
  try {
    ...
    return NextResponse.json(activityData, { headers: NO_STORE_HEADERS });
  } catch (error: unknown) {
    const errorMessage = error instanceof Error ? error.message : "An unknown error occurred";
    console.error("[API GET /github-activity] Error fetching GitHub activity data:", errorMessage);

    const errorView = createUnavailableUserActivityView({
      source: "error",
      error: "Failed to retrieve GitHub activity data.",
    });
    return NextResponse.json(errorView, { status: 500, headers: NO_STORE_HEADERS });
  }
}
