...
async function testRelatedContent() {
  ...
  const bookmark = allContent.find(c => 
    c.type === "bookmark" && c.tags.length > 0
  );
  ...
  for (const targetType of types) {
    ...
    scored.forEach((item, i) => {
      ...
      console.log(`      Breakdown: tag=${item.breakdown.tagMatch.toFixed(2)}, text=${item.breakdown.textSimilarity.toFixed(2)}, domain=${item.breakdown.domainMatch.toFixed(2)}, recency=${item.breakdown.recency.toFixed(2)}`);
    });
  }
  
  // Test a specific case: blog post about venture capital
  console.log("\n\n🧪 Specific Test: VC Blog Post");
  const vcBlog = allContent.find(c => 
    c.type === "blog" && c.title.toLowerCase().includes("venture")
  );
