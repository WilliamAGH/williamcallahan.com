...
const excludeCategories = excludeArg ? excludeArg.split("=")[1]?.split(",").map(s => s.trim().toLowerCase()) ?? [] : [];
const onlyCategories = onlyArg ? onlyArg.split("=")[1]?.split(",").map(s => s.trim().toLowerCase()) ?? [] : [];
...
async function generateAuditReport(
  ...
  const auditLog: AuditLog = {
    ...
    regenerationStats,
    duration,
    success: deletionStats.failedFiles === 0 && 
             Object.values(regenerationStats).every(stat => stat.success),
  };
  ...
  if (!isDryRun) {
    ...
    logger.info(`  Bookmarks: ${regenerationStats.bookmarks.success ? '✅' : '❌'} (${regenerationStats.bookmarks.count} items)`);
    logger.info(`  Slug Mappings: ${regenerationStats.slugMappings.success ? '✅' : '❌'} (${regenerationStats.slugMappings.count} items)`);
    logger.info(`  GitHub Activity: ${regenerationStats.github.success ? '✅' : '❌'} (${regenerationStats.github.count} items)`);
    logger.info(`  Logos: ${regenerationStats.logos.success ? '✅' : '❌'} (${regenerationStats.logos.count} items)`);
    logger.info(`  Content Graph: ${regenerationStats.contentGraph.success ? '✅' : '❌'} (${regenerationStats.contentGraph.count} items)`);
    logger.info(`  Search Indexes: ${regenerationStats.search.success ? '✅' : '❌'} (${regenerationStats.search.count} indexes)`);
  }
...
async function main() {
  ...
  const success = deletionStats.failedFiles === 0 && 
