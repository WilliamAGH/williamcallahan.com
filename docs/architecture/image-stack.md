...
| Next runtime       | `next.config.ts` (`images.localPatterns/remotePatterns/qualities/formats`)                                                                                                | Define which URLs `_next/image` may fetch, including local `/images/**`, enforce `minimumCacheTTL`, `dangerouslyAllowSVG`, and remote host allowlists derived from env-driven CALLAHAN hosts.                                                   |
...
| ---------------------------------------------------------------- | ---------------------------------------------------------------- | -------------------------------------------------- | ------------------------------------------------------- |
| `static-image-mapping.json` (`lib/data-access/static-images.ts`) | Maps legacy `/public/images/**` to hashed CDN URLs.              | Manually maintained                                | Placeholder utilities, SEO defaults, API fallbacks.     |
| `data/blog/cover-image-map.json`                                 | Maps blog post slugs to S3 keys for cover images.                | `scripts/sync-blog-cover-images.ts` (during CI)    | MDX frontmatter, blog rendering.                        |
