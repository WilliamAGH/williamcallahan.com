...
- Related content uses pgvector cosine ANN exclusively: `findSimilarByEntity()` (HNSW single-query) → `applyBlendedScoring()` (0.80 cosine + 0.10 recency + 0.10 quality) → `hydrateRelatedContent()` (batch domain hydration from 6 domain tables). The legacy heuristic engine (`content-similarity/`) has been removed.
