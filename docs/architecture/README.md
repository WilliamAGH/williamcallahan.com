...
- `/src/types/schemas/blog-frontmatter.ts` - Zod single owner of MDX frontmatter and canonical blog slugs; runtime MDX and Node seed ingestion parse through it
...
- Books use a PostgreSQL-first architecture: `lib/books/generate.ts` consolidates ABS data + manual enrichments + AI summaries into versioned `books_snapshots` rows with `books_latest` pointer updates; runtime reads come from DB-backed books queries, never calling ABS directly at request time. The scheduler container first runs `node --run update-data` to bootstrap its canonical default dataset and revalidates affected web caches before cron; the recurring `--books` job runs daily at 6 AM PT with cache invalidation through `/api/revalidate/books`.
...
- Scheduled GitHub refreshes enter through `src/lib/server/github-activity-refresh.ts`, which returns a successful no-op outside the production write environment and preserves Sentry reporting for unexpected production failures.
...
- Route protections: `src/proxy.ts` applies sitewide rate limiting, security headers, and optional Clerk protections for matched traffic.
...
- AI chat gateway contracts are SSE-only end-to-end (`/api/ai/chat/[feature]` plus `aiChat()` browser client), with shared feature identifiers/queue-key builders, native reasoning-delta forwarding, and marker-based Harmony channel-control sanitation centralized in `ai-shared-services` (see `architecture/ai-services.md`).
- Endpoint-compatible embeddings (Qwen3-Embedding-4B, 2560-d `halfvec`) are stored in the unified `embeddings` table (primary key: domain + entity_id) with one HNSW cosine index. Terminal upstream batch failures are checkpointed in `embedding_failures` until their retry time, so bookmark persistence continues. Dedicated data-updater cycles process one canonical batch and drain due checkpoints across later cycles instead of blocking scheduler health on the full backlog. Backfill via `node scripts/backfill-bookmark-embeddings.node.mjs` and `node scripts/backfill-domain-embeddings.node.mjs`. All database scripts MUST run under Node.js (not bun) per `AGENTS.md [RT1]`.
...
- Related content uses pgvector cosine ANN exclusively: `findSimilarByEmbedding()` (HNSW single-query) → `rankEmbeddingCandidates()` (0.70 cosine + 0.10 recency + 0.10 diversity + 0.10 quality) → `hydrateRelatedContent()` (batch domain hydration from 6 domain tables). The legacy heuristic engine (`content-similarity/`) has been removed.
- AI analysis latest/history reads and writes are PostgreSQL-backed through `ai_analysis_latest` and `ai_analysis_versions`; S3 is not part of the runtime analysis cache path. Present analysis rows use a non-expiring Next.js cache profile, missing rows are cached briefly, and successful writes immediately expire the analysis tags.
- Bookmarks support dual feed modes (Discover/Latest). Discover mode uses `getDiscoveryRankedBookmarks()` (82% recency + 18% engagement signal) with `applyCategoryDiversity()` to prevent same-category clustering. Engagement events flow from client → `POST /api/engagement` → `content_engagement` table → discovery score aggregation. The client uses SWR with `fallbackData: undefined` for discover mode to trigger a loading skeleton during API fetch.
...
| `next-js-16-usage`             | Governs Next.js 16, React 19, Vitest 4, and release-versioned static asset URLs.                                                                                         | [nextjs-framework.md](../standards/nextjs-framework.md) |                                              |
| `analytics`                    | Load Simple Analytics and Clicky tracking scripts in a safe, non-blocking, and privacy-conscious manner.                                                                  | [analytics.md](../features/analytics.md)                |                                              |
...
| `json-handling`                | Show how external JSON payloads are validated, persisted to PostgreSQL, cached, and projected to consumers.                                                              |                                                         | [Diagram](json-data.mmd)                     |
