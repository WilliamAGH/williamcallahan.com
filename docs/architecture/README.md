...
- Endpoint-compatible embeddings (Qwen3-Embedding-4B, 2560-d `halfvec`) are stored in `bookmarks`, `ai_analysis_latest`, `opengraph_metadata`, and `thoughts` tables with HNSW cosine indexes. Backfill via `node scripts/backfill-bookmark-embeddings.node.mjs` and `node scripts/backfill-domain-embeddings.node.mjs`. All database scripts MUST run under Node.js (not bun) per `CLAUDE.md [RT1]`.
