...
- Current production list: `s3-storage.callahan.cloud`, `williamcallahan.com`, `dev.williamcallahan.com`, plus wildcard entries for `*.digitaloceanspaces.com` and `**.digitaloceanspaces.com` so multi-level Spaces hosts such as `williamcallahan-com.sfo3.digitaloceanspaces.com` pass validation. Add any new Spaces/CDN hostname there **before** shipping content that references it.
